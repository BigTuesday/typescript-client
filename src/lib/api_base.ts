...
export class NludbTask<ResultType> {
  ...
  constructor(nludb: NludbApiBase, params: TaskStatusResponse, result?: ResultType) {
    this.nludb = nludb;
  ...
  async check() {
    const status = await (
      this.nludb.post("task/status", {taskId: this.taskId}, true) as Promise<NludbTask<TaskStatusResponse>>
    )
    this.update(status)
  }
  ...
  async _run_development_mode() {
    ...
    const status = await (
      this.nludb.post("task/next", {taskId: this.taskId}, true) as Promise<NludbTask<TaskStatusResponse>>
    )
    this.update(status)
  }

  async wait(params?: {maxTimeoutSeconds?: number, retryDelaySeconds?: number}): Promise<void> {
...
export class NludbApiBase {
  ...
  async post<T>(operation: string, payload: unknown, asynchronous = false): Promise<T | NludbTask<T>> {
    if (!(this.apiKey)) {
      throw new NLUDBError("Please set your NLUDB API key using the NLUDB_KEY environment variable!")
    }
    ...
    if (resp.data.status) {
      ...
      throw new NLUDBError("Neither data nor status property was present in task response")
    }
  }
}
